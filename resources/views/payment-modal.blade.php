...
(function() {
    ...
    const PRODUCT_ID = window.PAYMENT_DATA?.productId ?? {{ $productId ?? 'null' }};
    const USER_ID = window.PAYMENT_DATA?.userId ?? {{ $userId ?? 'null' }};
    ...
    window.xpmSelectMethod = function(code, name, tab) {
        ...
        .then(res => {
            ...
            currentOrderId = res.order_id ?? ORDER_ID;
    ...
    function renderQRIS(actions) {
        ...
        const webUrlAction = actions.find(a => a.descriptor === 'WEB_URL');
        const expiresAction = actions.find(a => a.descriptor === 'EXPIRY_DATE');
        ...
        if (qrString) {
            ...
            const fallbackUrl = webUrlAction?.value;
            canvas.innerHTML = fallbackUrl
                ? `<div style="padding:2rem 0;text-align:center;">
                        <p style="color:#666;font-size:12px;line-height:1.6;margin:0 0 12px;">QR belum bisa ditampilkan inline. Lanjutkan lewat halaman pembayaran Xendit.</p>
                        <a href="${fallbackUrl}" target="_blank" style="display:inline-block;padding:10px 14px;background:#111;color:#fff;border-radius:8px;text-decoration:none;font-size:13px;font-weight:600;">Buka Halaman Pembayaran</a>
                   </div>`
                : '<p style="color:#bbb;font-size:12px;text-align:center;padding:4rem 0;">QR tidak tersedia</p>';
        }
    }
    ...
    window.xpmCheckStatus = function() {
        ...
        .then(res => {
            ...
            if (res.status === 'SUCCEEDED') {
                ...
                setTimeout(() => {
                    window.location.href = `/checkout/success?order_id=${encodeURIComponent(res.order_id ?? currentOrderId)}`;
                }, 1500);
            }
