...
<div class="balance-card"
     style="background:linear-gradient(135deg,#1e40af,#2563eb);color:#ffffff;border-radius:20px;
    ...
    <div style="display:flex;justify-content:flex-end;">
        <button id="btn-withdraw" onclick="openWithdrawModal()"
                style="display:flex;align-items:center;gap:10px;padding:14px 22px;border-radius:14px;
                       border:none;cursor:pointer;font-size:15px;font-weight:700;background:#ffffff;
                       color:#1e40af;box-shadow:0 10px 30px rgba(0,0,0,.25);white-space:nowrap;">
...
{{-- ============================================================
     MODAL WITHDRAW — dirender di body level via @push('modals')
============================================================ --}}
@push('modals')
...
<div id="withdraw-modal"
    ...
    <div id="withdrawCard"
         style="background:#ffffff;width:100%;max-width:560px;max-height:90vh;border-radius:18px;
        ...
        <div style="padding:20px 24px;border-bottom:1px solid #eee;display:flex;align-items:center;justify-content:space-between;">
            <div style="display:flex;align-items:center;gap:10px;">
                <div style="width:36px;height:36px;border-radius:10px;background:#e0edff;display:flex;align-items:center;justify-content:center;color:#2563eb;">
                    <i class="fas fa-wallet"></i>
                </div>
                <h3 style="margin:0;font-size:18px;font-weight:700;">Tarik Saldo</h3>
            </div>
            <button onclick="closeWithdrawModal()"
                    style="border:none;background:none;font-size:24px;cursor:pointer;color:#6b7280;
        ...
        <div style="padding:24px;overflow-y:auto;">
            <div style="background:#eef6ff;border:2px solid #3b82f6;border-radius:14px;padding:16px;
                        margin-bottom:20px;font-size:14px;color:#1e40af;">
                <strong>Informasi Penarikan</strong><br>
                Saldo Tersedia: <b>Rp {{ number_format($balance ?? 0,0,',','.') }}</b><br>
                Minimal Penarikan: <b>Rp 50.000</b>
            </div>

            <form id="form-withdraw" onsubmit="handleWithdrawSubmit(event)" style="display:flex;flex-direction:column;gap:14px;">
                @csrf
                <div>
                    <label style="font-weight:600;font-size:14px;margin-bottom:6px;display:block;">Jumlah Penarikan (Rp) *</label>
                    <input type="number" name="amount" min="50000" max="{{ $balance ?? 0 }}"
                           placeholder="Contoh: 100000" required
                           style="width:100%;padding:12px;border:2px solid #e5e7eb;border-radius:10px;font-size:14px;">
                </div>
                <div>
                    <label style="font-weight:600;font-size:14px;margin-bottom:6px;display:block;">Nama Bank *</label>
                ...
                <div>
                    <label style="font-weight:600;font-size:14px;margin-bottom:6px;display:block;">Nomor Rekening *</label>
                    <input type="text" name="account_number" placeholder="1234567890" required
                           style="width:100%;padding:12px;border:2px solid #e5e7eb;border-radius:10px;font-size:14px;">
                </div>
                <div>
                    <label style="font-weight:600;font-size:14px;margin-bottom:6px;display:block;">Nama Pemilik Rekening *</label>
                    <input type="text" name="account_name" placeholder="Sesuai rekening bank" required
                           style="width:100%;padding:12px;border:2px solid #e5e7eb;border-radius:10px;font-size:14px;">
                </div>
                <div>
                    <label style="font-weight:600;font-size:14px;margin-bottom:6px;display:block;">Catatan (Opsional)</label>
                ...
                <div style="display:flex;gap:12px;margin-top:8px;">
                    <button type="button" onclick="closeWithdrawModal()"
                            style="flex:1;padding:14px;border-radius:10px;border:none;background:#f1f5f9;
                                   color:#374151;font-size:14px;font-weight:600;cursor:pointer;">
                        Batal
                    </button>
                    <button type="submit" id="submit-withdraw-btn"
                            style="flex:1;padding:14px;border-radius:10px;border:none;background:#2563eb;
...
async function handleWithdrawSubmit(event) {
    ...
    try {
        const res    = await fetch('/withdrawal', { method:'POST', headers:{'X-CSRF-TOKEN':token,'Accept':'application/json'}, body:formData });
        const result = await res.json();
        if (result.success) { alert(result.message); window.location.reload(); }
        else { alert(result.message || 'Gagal'); btn.disabled = false; btn.innerText = original; }
    } catch(e) { alert('Terjadi kesalahan'); btn.disabled = false; btn.innerText = original; }
}
